...
//*******************************************************************
void AH_EasyDriver::customSet(int RES, int DIR, int STEP, int MS1, int MS2, int SLP) {
    this->INIT_MODE = 1;
...
void AH_EasyDriver::move(int NUMBER_OF_STEPS) {
    ...
    while (STEPS > 0) {
        if (micros() - this->LAST_STEP_TIME >= (unsigned int) this->STEP_DELAY) {
